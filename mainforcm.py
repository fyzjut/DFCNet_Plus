...
class Processor():
    ...
    def start(self):
        tokenizer = MBartTokenizerFast.from_pretrained("facebook/mbart-large-cc25", use_auth_token=False, local_files_only=True)
        mbartencode = MBartModel.from_pretrained("facebook/mbart-large-cc25", use_auth_token=False, local_files_only=True)
        mbartencode = mbartencode.to('cuda')
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                seq_train(self.data_loader['train'], self.model, self.optimizer,
                          self.device, epoch, self.recoder,tokenizer,mbartencode)
                if eval_model:
                    dev_wer = seq_eval(self.arg, self.data_loader['dev'], self.model, self.device,
                                       'dev', epoch, self.arg.work_dir, self.recoder,tokenizer,mbartencode,self.arg.evaluate_tool)
            ...
            dev_wer = seq_eval(self.arg, self.data_loader["dev"], self.model, self.device,
                               "dev", 6667, self.arg.work_dir, self.recoder,tokenizer,mbartencode, self.arg.evaluate_tool)
            test_wer = seq_eval(self.arg, self.data_loader["test"], self.model, self.device,
                                "test", 6667, self.arg.work_dir, self.recoder, tokenizer,mbartencode,self.arg.evaluate_tool)
    ...
    def load_model_weights(self, model, weight_path):
       ...
       # print(f"model.alpha.data: {model.alpha.data.item():.10f}")
        # weights = self.modified_weights(state_dict['model_state_dict'])
        model.load_state_dict(weights, strict=True)
        #print(f"model.alpha.data: {model.V2TAdapter.alpha.data.item():.10f}")
