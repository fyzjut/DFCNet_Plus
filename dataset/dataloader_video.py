...
class BaseFeeder(data.Dataset):
    def __init__(self, prefix, gloss_dict, dataset='phoenix2014', drop_ratio=1, num_gloss=-1, mode="train", transform_mode=True,
                 datatype="lmdb", frame_interval=1, image_scale=1.0, kernel_size=1, input_size=224,subset_percent=100):
        self.mode = mode
        self.ng = num_gloss
        self.prefix = prefix
        ...
        # 在__init__方法或其他适当的地方
        #print("Keys example:", list(self.inputs_list.keys())[:10])

        if subset_percent < 100:
            self.inputs_list = self.subset_dict(self.inputs_list, subset_percent)
           # print(f"Reduced dataset size: {len(self.inputs_list)}")
    ...
    def subset_dict(self,data_dict, subset_percent):
        total_keys = list(data_dict.keys())
        subset_size = int(len(total_keys) * (subset_percent / 100))
        selected_keys = total_keys[:subset_size]  # 选择前subset_size个键
        #print("Selected keys:", selected_keys)  # 打印选中的键
    ...
    def __getitem__(self, idx):
        if self.data_type == "video":
            ...
            return input_data, torch.LongTensor(label), self.inputs_list[idx]['original_info'],fi
            ...
            return input_data, label, self.inputs_list[idx]['original_info'],fi
    ...
    def collate_fn(batch):
        ...
        video, label, info, fi = list(zip(*batch))
        ...
        if max(label_length) == 0:
            return padded_video, video_length, [], [], info,fi
        else:
            padded_label = []
            for lab in label:
                padded_label.extend(lab)
            padded_label = torch.LongTensor(padded_label)
            return padded_video, video_length, padded_label, label_length, info,fi
